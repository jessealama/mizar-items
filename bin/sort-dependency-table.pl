...
sub load_item_to_fragment_table {
    ...
    if (-d $item_to_fragment_table) {
	die 'Item-to-fragment table at ', $item_to_fragment_table, ' is actually a directory.';
    }

    if (! -e $item_to_fragment_table) {
	die 'Item-to-fragment table does not exist at ', $item_to_fragment_table;
    }

    if (! -r $item_to_fragment_table) {
	die 'Item-to-fragment table at ', $item_to_fragment_table, ' is unreadable.';
    }

    my %table = ();

    open (my $table_fh, '<', $item_to_fragment_table)
	or die 'Unable to open an input filehandle for ', $item_to_fragment_table, ': ', $!;
    ...
    close $table_fh
	or die 'Unable to close the input filehandle for ', $item_to_fragment_table, ': ', $!;
