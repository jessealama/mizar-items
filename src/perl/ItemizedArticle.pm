...
sub dependencies {
	...
	if (defined $dependencies{$item}) {
		...
		if ($dep =~ /\A ${article_name} [:] [^:]+ [:] [0-9]+ \z/) {
		    ...
		    if ($fragment_for_dep =~ /\A ${article_name} : fragment : ([0-9]+) /) {
			...
			my @redefinition_items = grep { / : [0-9]+ \[ (coherence | compatibility) \] \z/ } @generated_items;
			foreach my $redefinition_item (@redefinition_items) {
			    push (@more_deps, $redefinition_item);
			}
		    } else {
			croak ('Error: unable to make sense of the fragment \'', $fragment_for_dep, '\'.');
		    }

		}
	    }

	    $dependencies{$item} = \@more_deps;
	}
    }
	...
	if ($item =~ / : (.) constructor : ([0-9]+) \[ ( compatibility | coherence ) \] \z/) {
		...
		if ($fragment =~ /\A ${article_name} : fragment : ([0-9]+) /) {
		    ...
		    # Function case: ensure that the existence and
		    # uniqueness items for the constructor depend on
		    # the coherence/compatibility condition
		    if ($item_kind eq 'k') {
			my $constructor_item = "${article_name}:kconstructor:${item_number}";
			my $item_escaped = escape_item ($item);
			for my $condition ('existence', 'uniqueness') {
			    my $condition_item = "${constructor_item}[${condition}]";
			    if (defined $dependencies{$condition_item}) {
				my @deps = @{$dependencies{$condition_item}};
		...
		croak ('Error: ', $item, ' is missing from the item-to-fragment table.');
	    }
	}
    }
