...
(defgeneric fragment->items (fragment)
  (:documentation "The list of items generated by FRAGMENT."))

(let ((fragment-items-table (make-hash-table :test #'equal)))
  (defmethod fragment->items :around (fragment)
    (multiple-value-bind (items present?)
	(gethash fragment fragment-items-table)
      (if present?
	  items
	  (let ((new-items (call-next-method)))
	    (setf (gethash fragment fragment-items-table)
		  new-items))))))

(defmethod fragment->items (fragment)
