  ...
  (register-groups-bind (article-name item-kind item-number)
	...
	(let* ((ckb-item-path (format nil "~a/ckb~d.html"
	  ...
	  (miz-item-html (str item-key)
	    (:p (str item-key) " comes from article fragment " (str ckb-number) " of article " (str article-name) ".")
	    (if (null (cdr items-for-ckb)) ; the CKB for this item generates only this item
		(htm 
		 (:p "This is the only item generated by that article fragment."))
		
		(htm
		 (:p "This article fragment generates this item, as well as these other items:")
		 (:ul
